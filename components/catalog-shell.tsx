...
export function CatalogHeader(){const{lang,t,toggle}=useCatalogLanguage();return <header className="field-top"><a href="/" aria-label={t(['返回乡土国际首页','Return to Countryside International home'])}>乡土国际 / CI</a><Link href="/about">{t(['关于乡土国际','About'])}</Link><button onClick={toggle} aria-label="Switch language">{lang?'中文 / EN':'中 / English'}</button></header>}
