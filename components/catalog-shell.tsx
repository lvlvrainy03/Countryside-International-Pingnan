...
export function CatalogNav({home=false}:{home?:boolean}){
 ...
 return <><div ref={marker} className="nav-marker"/><nav className={`catalog-nav ${compact?'is-compact':''}`} aria-label={t(['内容分类','Content categories'])}>{sections.map((s,i)=><Link href={`/${s.id}`} key={s.id} aria-current={path.startsWith('/'+s.id)?'page':undefined}><span className="nav-number">0{i+1}</span><span>{t(s.name)}<small>{s.name[1]}</small></span></Link>)}</nav></>;
}
