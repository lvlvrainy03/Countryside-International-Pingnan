...
export default function FieldAtlas(){
 ...
 <div className="directory-grid">{sections.map(s=><article className="directory-column" key={s.id}><p>{t(s.intro)}</p>{s.id==='people'?<><Link className="directory-item" href="/people"><small>40 / LOCAL CO-CREATORS</small>{t(['大食物共创者','Food co-creators'])}</Link><Link className="directory-item" href="/people#international">{t(['国际教师与学生','International teachers & students'])}</Link><Link className="directory-item" href="/people#academy">{t(['美院师生团队','CAA teachers & students'])}</Link></>:records.filter(r=>r.section===s.id).slice(0,3).map(r=><Link className="directory-item" href={recordHref(r)} key={r.slug}><small>{r.date}</small>{t(r.title)}</Link>)}<Link className="directory-more" href={'/'+s.id}>{t(['浏览全部','View all'])} ↗</Link></article>)}</div>
